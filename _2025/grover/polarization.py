...
from _2025.blocks_and_grover.axes_patch import PatchedThreeDAxes
...
class BeamSplitter(InteractiveScene):
    def construct(self):
        ...
        top_axes, low_axies = split_axes = VGroup(PatchedThreeDAxes(), PatchedThreeDAxes())
    ...
    def get_wave(
        ...
        density = 0.1
        # Configure the axes to have the right density
        axes.x_axis.x_step = density

        # Set the x range based on max_x
        axes.x_axis.x_min = 0
        axes.x_axis.x_max = max_x

        wave = TimeVaryingVectorField(
            field_func,
            axes,
            density=1.0/density,  # Use density parameter instead of x_density
            max_vect_len=max_vect_len,
            stroke_color=color,  # Use stroke_color instead of color
